...
def show_regions_list():
    ...
    for region_code in Regions.keys():
        print(
            f"Region Name: {Regions[region_code]['Region']}\tRegion Code: {region_code}"
        )
...
class GoogleAds:
    def __init__(self, region="anywhere", proxy=None):
        ...
        if (not Regions.get(region)) and self.r_check:
            raise ValueError(
                f"Region: {region} is not supported, Please call the show_region_list() function to get supported regions list"
            )
        self.region = region
    ...
    def get_cookies(self):
        ...
        self.reqs.get(
            "https://adstransparency.google.com/", params=params, headers=self.headers
        )  # .text.replace("\/","")
    ...
    def creative_search_by_advertiser_id(
        self, advertiser_id: str, count: int = 40, next_page_id: str = ""
    ) -> list:  # TODO do region search
        ...
        count -= len(ads)
        ads.extend(
            self.creative_search_by_advertiser_id(advertiser_id, count, next_page_id)
        )
        return ads
        ...
        if search.get("2"):
            ...
            search = next(
                (
                    ...
                    if suggestion["1"]["1"] == advertisor["Name"]
                    and suggestion["1"]["2"] == advertisor["Advertisor Id"]
                ),
                None,
            )
    ...
    def get_link_to_video(self, link: str) -> str:
        ...
        try:
            txt = next(
                (x for x in response.text.split("CDATA[") if "googlevideo.com" in x)
            )
    ...
    def get_breif_ads(
        self, advertisor_id: str, creative_id: str
    ) -> dict:  # TODO do region search
        ...
        if "1" not in response.json():
            return {
                "Advertisor Id": advertisor_id,
        ...
        date = datetime.datetime.fromtimestamp(int(response["4"]["1"])).strftime(
            "%Y-%m-%d"
        )
        return {
            "Adverisor Id": advertisor_id,  # TODO Add Country
            "Creative Id": creative_id,
            "Ad Format": format_,
            "Last Shown": date,
            "Ad Link": link,
    ...
    def parse_ad_link(self, html: str) -> dict:
        ...
        result.extend(
            [
                "".join([_y for _y in y if _y in string.printable])
                for y in [
                    x.text
                    for x in page.find_all(
                        "span", "nNzjpf-cg9Tke-KZWGte-OuLqWe nNzjpf-twKXnc-mpzrze"
                    )
                ]
            ]
        )
        script_link_code = re.findall(
            r"var AF_dataServiceRequests = (.*) var AF_initDataChunkQueue",
            html.encode().decode("unicode_escape"),
        )
        for _possibility in script_link_code:
            result.extend(re.findall(url_regex, _possibility))
    ...
    def get_detailed_ad(self, advertisor_id: str, creative_id: str) -> dict:
        ...
        if not ad_detail["Ad Link"]:
            return ad_detail

        response = self.reqs.get(ad_detail["Ad Link"])
        ad_detail.update(
            {"Ad Body": "", "Ad Title": "", "Image URL": "", "Video URL": ""}
        )
